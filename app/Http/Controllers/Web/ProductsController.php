...
class ProductsController extends Controller
{
    ...
    public function accessObjectArray($var)
    {
     ...
     public function productDetail(Request $request)
     {
		...
		$result['author'] = DB::table('users')->where('id', $result['detail']['product_data'][0]->products_author)->first();
		
		$min_price = '';$max_price = '';$limit = 15000;$type = '';$categories_id = '';$search = '';
		  $data = array('page_number' =>'0', 'type' => $type, 'limit' => $limit, 'categories_id' => $categories_id, 'search' => $search, 'filters' => [], 'limit' => $limit, 'min_price' => $min_price, 'max_price' => $max_price,"author_id"=>$result['detail']['product_data'][0]->products_author);
		  
		  
		  $author_products = $this->products->products($data);
		 $result['totallikes']=$result['totalreviews']=$result['totalviewed']=$result['totalordered']=0;
		 
		 $result['totalrecipes'] = count($author_products['product_data']);
		  foreach($author_products['product_data'] as $prod)
		  {
